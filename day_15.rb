...
def part_2
  $instructions.each do |instruction|
    case instruction
    when "<"
      if can_move_2?($robot_position, instruction)
        move_2($robot_position, instruction)
        $robot_position[1] -= 1 
      end
    when ">"
      if can_move_2?($robot_position, instruction)
...
def move_2(position,instruction)
  x, y = position
  case instruction
  when "<"
    if $walls[[x, y-1]]
  ...
  when "^"
    if $walls[[x-1, y]]
      ...
      move_double_box_up?([x-1, y-1], [x-1, y])
      $right_boxes[[x-1, y]] = false
      $left_boxes[[x-1, y-1]] = false
      $right_boxes[[x-2, y]] = true
      $left_boxes[[x-2, y-1]] = true
    elsif $left_boxes[[x-1, y]]
      move_double_box_up?([x-1, y], [x-1, y+1])
        $left_boxes[[x-1, y]] = false
        $right_boxes[[x-1, y+1]] = false
        $left_boxes[[x-2, y]] = true
        $right_boxes[[x-2, y+1]] = true
  ...
  when "v"
    if $walls[[x+1, y]]
      ...
      move_double_box_down?([x+1, y-1], [x+1, y])
      $right_boxes[[x+1, y]] = false
      $left_boxes[[x+1, y-1]] = false
      $right_boxes[[x+2, y]] = true
      $left_boxes[[x+2, y-1]] = true
    elsif $left_boxes[[x+1, y]]
      move_double_box_down?([x+1, y], [x+1, y+1])
      $left_boxes[[x+1, y]] = false
      $right_boxes[[x+1, y+1]] = false
      $left_boxes[[x+2, y]] = true
      $right_boxes[[x+2, y+1]] = true
...
def move_double_box_up?(position1, position2)
  x1, y1 = position1
  x2, y2 = position2
  left_can_move_up = true
  right_can_move_up = true
  ...
  if $right_boxes[[x1-1, y1]]
    move_double_box_up?([x1-1, y1-1], [x1-1, y1])
    $right_boxes[[x1-1, y1]] = false
    $left_boxes[[x1-1, y1-1]] = false
    $right_boxes[[x1-2, y1]] = true
    $left_boxes[[x1-2, y1-1]] = true
  ...
  if $left_boxes[[x2-1, y2]]
    move_double_box_up?([x2-1, y2], [x2-1, y2+1])
    $left_boxes[[x2-1, y2]] = false
    $right_boxes[[x2-1, y2+1]] = false
    $left_boxes[[x2-2, y2]] = true
    $right_boxes[[x2-2, y2+1]] = true
  ...
  if $left_boxes[[x1-1, y1]] && $right_boxes[[x2-1, y2]]
    move_double_box_up?([x1-1, y1], [x2-1, y2])
    $left_boxes[[x1-1, y1]] = false
    $right_boxes[[x2-1, y2]] = false
    $left_boxes[[x1-2, y1]] = true
    $right_boxes[[x2-2, y2]] = true
...
def move_double_box_down?(position1, position2)
  x1, y1 = position1
  x2, y2 = position2
  left_can_move_down = true
  right_can_move_down = true
  ...
  if $right_boxes[[x1+1, y1]]
    move_double_box_down?([x1+1, y1-1], [x1+1, y1])
    $right_boxes[[x1+1, y1]] = false
    $left_boxes[[x1+1, y1-1]] = false
    $right_boxes[[x1+2, y1]] = true
    $left_boxes[[x1+2, y1-1]] = true
  ...
  if $left_boxes[[x2+1, y2]]
    move_double_box_down?([x2+1, y2], [x2+1, y2+1])
    $left_boxes[[x2+1, y2]] = false
    $right_boxes[[x2+1, y2+1]] = false
    $left_boxes[[x2+2, y2]] = true
    $right_boxes[[x2+2, y2+1]] = true
  ...
  if $left_boxes[[x1+1, y1]] && $right_boxes[[x2+1, y2]]
    move_double_box_down?([x1+1, y1], [x2+1, y2])
    $left_boxes[[x1+1, y1]] = false
    $right_boxes[[x2+1, y2]] = false
    $left_boxes[[x1+2, y1]] = true
    $right_boxes[[x2+2, y2]] = true
